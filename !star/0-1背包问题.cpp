...
int select[MAX_N + 1]; // 用于存放最优解（能使放入背包的物品的总价值最大的放置方案）
...
void Knap(int N, int W, int *w, int *v) // 动态规划法求解0/1背包问题
{
    ...
    for (int i = 1; i <= N; i++)
    {
        for (int r = 1; r <= W; r++) // r是包的剩余载重量
        {
            ...
            else // 当 **背包剩余载重量r**>=**物品i的重量w[i]** 时。在**不放入**和**放入**物品i之间选最优解（能使放入背包的物品的总价值最大的放置方案）
            {
                dp[i][r] = max(
                    dp[i - 1][r],
                    dp[i - 1][r - w[i]] + v[i]);
...
int Buildx(int N, int W, int *w, int *v) // 推导出最优解对应的解向量x
{
    int maxv = 0;
    int i = N, r = W; // r是包的剩余载重量
    while (i >= 0)    // 判断在该放置方案中，物品i是否要被放入包中
    {
        if (dp[i][r] != dp[i - 1][r])
        {                     // 表示放入物品i
            select[i] = true; // 选取物品i
...
int main()
{
    ...
    Knap(N, W, w, v);
    int maxv = Buildx(N, W, w, v); // 在最优解（能使放入背包的物品的总价值最大的放置方案）中，最终放入背包的物品的总价值
