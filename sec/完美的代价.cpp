...
int main()
{
    ...
    for (int i = 0; i < j; i++)
    {
        for (int k = j; k >= i; k--) // k下标从第j个字符起，一路往前到i，目的是寻找和str[i]相同的字符：str[k]
        {
            if (k == i) //如果找不到和str[i]相同的字符
            {
                ...
                // 那么交换成回文的时候其中一个字符就要经过中间，这样每处理一对这样的字符，cnt就会被多加一个1，而这个1是没有必要的，因为你完全可以等除了这个中间字符以外的所有字符移动完了之后再把这个中间字符移动过去，只有这样的交换策略才能保证交换次数最少。
                cnt += (N / 2 - i);
            }
            else if (str[k] == str[i]) // 如果找到了和str[i]相同的字符：str[k]
            {
                for (int l = k; l < j; l++) //把str[k]交换到j下标所指向的位置
                {
                    swap(str[l], str[l + 1]);
                    cnt++; // 交换次数加1
                }
