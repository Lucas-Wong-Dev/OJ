...
int main()
{
    ...
    int daysUnvisited = 0;
    int arr[] = {0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31}; // 一年中每个月的天数
    int i = 12; // 这个i就是arr数组的下标
    while (daysVisited <= gap)
    {
        if (i > 12) // 如果已经遍历完了当前年份
        {
            i = 1;  // 将表示月份的变量i重新置为1，也就是将月份重新置为一月份（注意这里的一月份是新的一年的一月份）
        ...
        daysVisited += arr[i];
        month = i; // 更新高斯获得博士学位的那天所在的月份
        i++;
    }
    printf("%d-%d-%d\n", year, month, day + daysUnvisited); // 天数还要加上原先的15天
    return 0;
}
