...
// #include <stdio.h> // AC
// 注意：虽然这个代码在OJ上AC了，但实际上这种写法是存在问题的，因为字符串数组str的长度太短了，当输入为26时，问题就显露出来了
// #include <string.h>
// int main()
// {
//     int N;
...
//     scanf("%d", &N);
//     for (int i = 1; i < N; i++) // 注意：对A1进行一次处理即可得到A2，再对A2进行一次处理即可得到A3，也就是说为了得到A3，我们要进行两次处理。每次循环期间都进行了一次处理，因此这里循环结束条件为i<N
//     {
