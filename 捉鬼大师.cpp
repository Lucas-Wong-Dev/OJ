...
1.	输入：
第一行：给出两个数字 g,b 对应于鬼的个数和捉鬼大师的个数。
接下来的g行，每行两个整数，是每个鬼的坐标（之间用空格隔开），
在接下来的b行，每行两个整数，是每个捉鬼大师的坐标（之间用空格个隔开），不会有重复的鬼和捉鬼大师出现。
2.	输出：
...
void result(GXY *gArr, BXY *bArr)
{
    for (int i = 0; i < b; i++)
    {
        for (int j = 0; j < g; j++)
        {
            if ((gArr[j].x > bArr[i].y && gArr[j].y < bArr[i].y) && j < g - 1)
            {
                gArr[j].x = gArr[j + 1].x;
                gArr[j].y = gArr[j + 1].y;
                g--;
    ...
    for (int i = 0; i < g - 1; i++) //排序
    {
        for (int j = 0; j < g - 1 - i; j++)
        {
            ...
            if (gArr[j].y == gArr[j + 1].y && gArr[j].x > gArr[j + 1].x)
            {
                GXY temp;
                temp = gArr[j];
                gArr[j] = gArr[j + 1];
