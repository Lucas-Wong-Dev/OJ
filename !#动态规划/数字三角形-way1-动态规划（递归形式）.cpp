...
// aMaxSum(i, j):代表从第i行的第j个数字到底边的最佳路径上的数字之和，则本题是求aMaxSum(1, 1)
...
{
    // 本函数的返回值：从位于(i, j)的结点开始，到底边路径的最大和
    if (i == N)
        return D[i][j];
...
int main()
{
    ...
    memset(aMaxSum, -1, sizeof(aMaxSum)); // 将 aMaxSum 全部置成-1, 开始时所有的 MaxSum(N, i, j)都没有算过
