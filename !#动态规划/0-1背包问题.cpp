...
void Knap(int N, int W, int *weight, int *value) // 动态规划法求解0-1背包问题
{
    ...
    for (int i = 1; i <= N; i++)
    {
        ...
        {
            if (r < weight[i]) // 当 背包剩余载重量r<物品i的重量weight[i] 时，物品i放不下
            {
                dp[i][r] = dp[i - 1][r];
            }
            else // 当 背包剩余载重量r>=物品i的重量weight[i] 时。在 不放入 和 放入 物品i之间选最优解
            {
                dp[i][r] = max(
                    dp[i - 1][r],
...
int Buildx(int N, int W, int *weight, int *value, int *totalWeightOfLoadedObjects) // 推导出最优解对应的解向量x
{
    ...
    {
        if (dp[i][r] == dp[i - 1][r]) // 表示物品i放不下或者不放入物品i
        {
...
int main()
{
    ...
    // 共有N件物品，包的最大载重量为W
    // 求解：放入哪些物品能使得总价值最高
    int weight[MAX_N + 1] = {0, 2, 2, 6, 5, 4}; // 物品i的重量。下标0不用
    int value[MAX_N + 1] = {0, 6, 3, 5, 4, 6};  // 物品i的价值。下标0不用

    Knap(N, W, weight, value);

    int totalWeightOfLoadedObjects = 0;
    int maxv = Buildx(N, W, weight, value, &totalWeightOfLoadedObjects); // 在最优解（能使放入背包的物品的总价值最大的放置方案）下，最终放入背包的物品的总价值
    ...
    //     {
    //         Knap(n1, w1, weight, value);
    //         int maxv = Buildx(n1, w1, weight, value);
